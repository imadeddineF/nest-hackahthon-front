...
import { getRotationFromDirection, warehouseDimensions, pathWaypoints } from "../lib/path-utils"
...
export default function Warehouse3D({
  ...
  const colors = {
    ...
    path: isDarkMode ? "#3b82f6" : "#2563eb", // Blue path color
  }
  ...
  return (
    <div className="w-full h-full">
      <Canvas shadows camera={{ position: [0, 30, 30], fov: 50 }}>
        ...
        {/* Path Visualization */}
        <PathVisualization pathColor={colors.path} />
...
// Update the PathVisualization component to include the perimeter path
function PathVisualization({ pathColor }: { pathColor: string }) {
  ...
  const mainPathColor = pathColor
  const perimeterPathColor = "#f59e0b" // Amber color for perimeter path

  // Define perimeter path waypoints
  const perimeterPathWaypoints = [
    { x: 5, z: 5 },
    { x: 115, z: 5 },
    { x: 115, z: 95 },
    { x: 5, z: 95 },
    { x: 5, z: 5 }, // Close the loop
  ]
  ...
  return (
    <group>
      {/* Main Path */}
      <group>
        {/* Path waypoints as glowing spheres */}
        {pathWaypoints.map((point, index) => (
          <mesh key={`waypoint-${index}`} position={[point.x + offsetX, 0.1, point.z + offsetZ]}>
            <sphereGeometry args={[0.5, 16, 16]} />
            <meshStandardMaterial
              color={mainPathColor}
              emissive={mainPathColor}
        ...
        {pathWaypoints.map((point, index) => {
          if (index < pathWaypoints.length - 1) {
            const start = pathWaypoints[index]
            const end = pathWaypoints[index + 1]

            // Calculate midpoint and direction
            const midX = (start.x + end.x) / 2 + offsetX
            const midZ = (start.z + end.z) / 2 + offsetZ
            ...
            return (
              <mesh key={`path-line-${index}`} position={[midX, 0.05, midZ]} rotation={[0, -angle, 0]}>
      ...
      {/* Perimeter Path */}
      <group>
        {/* Perimeter path waypoints */}
        {perimeterPathWaypoints.map((point, index) => (
          <mesh key={`perimeter-waypoint-${index}`} position={[point.x + offsetX, 0.1, point.z + offsetZ]}>
            <sphereGeometry args={[0.5, 16, 16]} />
            <meshStandardMaterial
              color={perimeterPathColor}
              emissive={perimeterPathColor}
              emissiveIntensity={0.5}
              transparent
              opacity={0.7}
            />
        ...
        {/* Perimeter path lines */}
        {perimeterPathWaypoints.map((point, index) => {
          if (index < perimeterPathWaypoints.length - 1) {
            const start = perimeterPathWaypoints[index]
            const end = perimeterPathWaypoints[index + 1]
            ...
            return (
              <mesh key={`perimeter-line-${index}`} position={[midX, 0.05, midZ]} rotation={[0, -angle, 0]}>
                <boxGeometry args={[length, 0.1, 0.3]} />
                <meshStandardMaterial
                  color={perimeterPathColor}
                  emissive={perimeterPathColor}
...
function WarehouseLayout({
  ...
  return (
    <group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[offsetX + 30, 0.01, offsetZ + 23]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[offsetX + 75, 0.01, offsetZ + 23]}>
        <planeGeometry args={[0.5, 46]} />
        <meshStandardMaterial
          color={pathColor}
          metalness={0.3}
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[offsetX + 52.5, 0.01, offsetZ + 46]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[offsetX + 52.5, 0.01, offsetZ + 73]}>
  ...
  return (
    <group ref={group} position={[robot.position.x + offsetX, robot.position.y, robot.position.z + offsetZ]}>
      ...
      <Html position={[0, 2.5, 0]} center distanceFactor={10}>
        <div
          className={`${
            isDarkMode ? "bg-gray-800/90" : "bg-blue-900/90"
          } backdrop-blur-md px-3 py-2 rounded-md text-white text-xs font-medium whitespace-nowrap shadow-lg`}
        >
