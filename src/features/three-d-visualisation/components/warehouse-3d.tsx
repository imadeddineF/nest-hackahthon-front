		...
		<group>
			...
			<Wall
				position={[
					offsetX,
					wallHeight / 2,
					warehouseDimensions.height / 2 + offsetZ,
				]}
			...
			<Wall
				position={[
					offsetX + warehouseDimensions.width,
					wallHeight / 2,
					warehouseDimensions.height / 2 + offsetZ,
				]}
				size={[wallThickness, wallHeight, warehouseDimensions.height]}
			...
			<mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
				<planeGeometry
					args={[warehouseDimensions.width, warehouseDimensions.height]}
				/>
				<meshStandardMaterial
					color={floorColor}
					metalness={0.2}
	...
	useFrame((_, delta) => {
		if (group.current) {
			...
			group.current.position.x +=
				(targetX - group.current.position.x) * 5 * delta;
			group.current.position.y +=
				(targetY - group.current.position.y) * 5 * delta;
			group.current.position.z +=
				(targetZ - group.current.position.z) * 5 * delta;
